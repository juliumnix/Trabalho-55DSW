...
import { Container, TableData, TableDataEntrega, TableHeader, TableProduct } from "./styles";
...
export default function DroneHistoryScreen() {
    ...
    function vendas(item) {
        ...
        item.vendas.forEach(element => {
            ...
            lista.push(
                <TableData>
                    {stringNomeProd(element.produtos)}
                </TableData>
            );
            lista.push(
                <TableData>
                    {stringQuantidadeProd(element.produtos)}
                </TableData>
            );
            lista.push(
                <TableData>
                    {stringTamanhoProd(element.produtos)}
                </TableData>
            );
            lista.push(<TableDataEntrega style={{color:converterEntregue(element.entregue)}}>{element.entregue ? "Entregue" : "Não Entregue"}</TableDataEntrega>);
        });
        return lista;
    }
